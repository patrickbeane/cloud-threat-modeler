...
class MarkdownReportTests(unittest.TestCase):
    ...
    def test_report_renders_analysis_coverage_for_auditing(self) -> None:
        ...
        self.assertIn("- Registered provider rules (AWS): `100`", report)
        self.assertIn("- Enabled provider rules (AWS): `100`", report)
...
class JsonReportTests(unittest.TestCase):
    ...
    def test_json_report_auto_selects_gcp_inventory_fixture(self) -> None:
        ...
        self.assertEqual(payload["summary"]["active_findings"], 26)
        self.assertEqual(payload["summary"]["severity_counts"], {"high": 6, "medium": 20, "low": 0})
    ...
    def test_json_report_includes_analysis_coverage_for_auditing(self) -> None:
        ...
        self.assertEqual(coverage["rules"]["registered_rule_count"], 100)
        self.assertIn("aws-database-permissive-ingress", coverage["rules"]["enabled_rules"])
        self.assertTrue(all(rule_id.startswith("aws-") for rule_id in coverage["rules"]["enabled_rules"]))
