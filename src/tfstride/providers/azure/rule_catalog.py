...
AZURE_RULE_METADATA = (
    ...
    RuleMetadata(
        rule_id="azure-public-app-service-service-bus-message-disruption",
        title="Public Azure App Service identity can disrupt Service Bus messages",
        category=StrideCategory.DENIAL_OF_SERVICE,
        recommended_mitigation=(
            "Remove Azure Service Bus Data Receiver, unnecessary Azure Service Bus Data Owner, and equivalent "
            "custom-role receive-and-settle permissions from public App Service runtime identities. Scope required "
            "access to exact queues or subscriptions, separate message consumers from public request-handling "
            "workloads where practical, and restrict public ingress."
        ),
        tags=(
            "azure",
            "app-service",
    ...
    RuleMetadata(
        rule_id="azure-public-app-service-service-bus-topology-disruption",
        title="Public Azure App Service identity can disrupt Service Bus topology",
        category=StrideCategory.DENIAL_OF_SERVICE,
        recommended_mitigation=(
            "Remove Service Bus namespace, queue, topic, or subscription delete authority from public App Service "
            "runtime identities. Scope required control-plane access to the exact topology resource, preserve "
            "management locks where appropriate, and restrict public ingress."
        ),
        tags=(
            "azure",
            "app-service",
    ...
    RuleMetadata(
        rule_id="azure-public-app-service-key-vault-key-disruption",
        title="Public Azure App Service identity has Key Vault key-disruption authority",
        category=StrideCategory.DENIAL_OF_SERVICE,
        recommended_mitigation=(
            "Restrict public App Service ingress and grant its runtime identity only the exact Key Vault key-update, "
            "deletion, or purge operations and scopes required. Enable purge protection where permanent key removal "
            "is not an intended workload capability."
        ),
        tags=(
            "azure",
            "app-service",
    ...
    RuleMetadata(
        rule_id="azure-public-app-service-key-vault-authorization-delegation",
        title="Public Azure App Service identity has Key Vault authorization-delegation authority",
        category=StrideCategory.ELEVATION_OF_PRIVILEGE,
        recommended_mitigation=(
            "Restrict public App Service ingress and grant its runtime identity only the exact Azure RBAC role-assignment "
            "or legacy Key Vault access-policy management authority required. Keep authorization-model transition "
            "authority separate from routine data-plane key access."
        ),
        tags=(
            "azure",
            "app-service",
