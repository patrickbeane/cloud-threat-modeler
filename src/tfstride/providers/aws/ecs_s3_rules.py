...
class AwsEcsS3AccessRuleDetectors:
    ...
    def detect_public_service_mutation_access(
        ...
        for service in context.inventory.by_type(_AWS_ECS_SERVICE):
            ...
            bucket_addresses = path_string_values(mutation_paths, "bucket_address")
            ...
            findings.append(
                self._finding_factory.build(
                    ...
                    rationale=(
                        ...
                        f"{len(bucket_addresses)} exact modeled S3 bucket(s). A compromise of the public workload "
                        "could modify or delete stored objects, or change bucket controls when administrative "
                    ...
                    evidence=collect_evidence(
                        ...
                        evidence_item("s3_mutation_paths", _mutation_path_evidence(mutation_paths)),
                    ),
...
def _is_deterministic_mutation_path(path: Mapping[str, Any], service_address: str) -> bool:
    return (
        ...
        and all(
            ...
            for key in ("task_definition_address", "role_address", "bucket_address", "bucket_arn")
        )
